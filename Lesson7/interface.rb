class Interface
  ...
  def run
    ...
    puts '- (10)Viev a railway carriages list.'
    puts '- (11)Viev a trains list on station.'
    puts '- (12)Take up space in the ailway carriage'
    puts '##################################'
    ...
    case answer
      ...
      when 10 then railway_carriages_list
      when 11 then trains_on_station_list
      when 12 then take_the_place
    end
  end
  ...
  def add_railway_carriages
    ...
    answer = gets.chomp.to_i
    puts "Enter number of available seats(if passenger) or available volume(if cargo)."
    answer1 = gets.chomp.to_i
    if Train.all[answer - 1].type == :cargo
      Train.all[answer - 1].add_railway_carriage(CargoRailwayCarriage.new(answer1))
    elsif Train.all[answer - 1].type == :passenger
      Train.all[answer - 1].add_railway_carriage(PassengerRailwayCarriage.new(answer1))
    end
  ...
  def railway_carriages_list
    puts "Choose the train."
    Train.all.each_with_index { |t, i| puts "#{i + 1}. #{t.type.to_s.capitalize} train, number #{t.number}" }
    answer = gets.chomp.to_i
    Train.all[answer - 1].railway_carriages_to_block { |rc, i| puts "Number: #{i + 1}. Railway carriage type: #{rc.show_type.to_s}. Occupied place #{rc.filled_volume}/#{rc.empty_volume}." }
  end

  def trains_on_station_list
    puts 'Choose station:'
    Station.all.each_with_index { |s, i| puts "#{i + 1}. #{s.show_title.capitalize}" }
    answer = gets.chomp.to_i
    Station.all[answer - 1].trains_to_block { |train| puts "Train number #{train.number}, type #{train.type}, railway carrigages: #{train.show_railway_carriage_count}" }
  end
  ...
  def take_the_place
    ...
    Train.all.each_with_index { |t, i| puts "#{i + 1}. #{t.type.to_s.capitalize} train, number #{t.number}" }
    answer = gets.chomp.to_i
    puts "Choose railway carriage"
    Train.all[answer - 1].railway_carriages_to_block { |rc, i| puts "#{i + 1}. Occupied place #{rc.filled_volume}/#{rc.empty_volume}." }
    answer1 = gets.chomp.to_i
    if Train.all[answer - 1].show_railway_carriages[answer1 - 1].show_type == :passenger
      Train.all[answer - 1].show_railway_carriages[answer1 - 1].get_place
    elsif Train.all[answer - 1].show_railway_carriages[answer1 - 1].show_type == :cargo
      print "Enter volume:"
      answer2 = gets.chomp.to_i
      Train.all[answer - 1].show_railway_carriages[answer1 - 1].fill_volume(answer2)
    end
  end
end
